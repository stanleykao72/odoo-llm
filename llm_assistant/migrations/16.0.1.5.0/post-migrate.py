    ...
    with api.Environment.manage():
        ...
        for assistant in assistants:
            ...
            if not assistant.category_id:
                _logger.warning(f"Assistant '{assistant.name}' (ID: {assistant.id}) has no category")
                continue
            ...
            if code_parts:
                ...
                _logger.info(f"Set code '{new_code}' for assistant '{assistant.name}' (ID: {assistant.id})")
            else:
                _logger.warning(f"Could not generate code for assistant '{assistant.name}' (ID: {assistant.id})")
        
        # Commit the changes
        cr.commit()
        
        _logger.info("Migration completed: Generated dotted codes for assistants based on category hierarchy")