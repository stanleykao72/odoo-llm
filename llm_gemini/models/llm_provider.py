...
class LLMProvider(models.Model):
    ...
    def _check_gemini_user_permission(self):
        ...
        # For now, only admin users have access (minimal implementation)
        if not current_user.has_group('base.group_system'):
            raise AccessError("User does not have permission to access Gemini services")
        
        # Also check if user is active and not an internal user
